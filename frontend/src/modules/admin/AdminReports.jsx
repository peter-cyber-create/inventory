  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold text-gov-primary">System Reports</h1>
        <button type="button" onClick={load} disabled={loading} className="ims-btn-secondary disabled:opacity-50">
          Refresh
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {cards.map(({ label, value }) => (
          <div key={label} className="bg-white rounded-lg shadow border p-4">
            ...
            <dd className="mt-1 text-2xl font-semibold text-gov-primary">{value}</dd>
          </div>
        ))}
      </div>
    </div>
