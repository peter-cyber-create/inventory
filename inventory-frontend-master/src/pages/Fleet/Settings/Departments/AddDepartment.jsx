...
const AddType = ({ close, refresh }) => {
    ...
    const [formData, setFormData] = useState({ deptHead: "", name: "" });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await API.post("/depts", formData);
            ...
            <div class="card-body">
                <section id="kyc-verify-wizard-p-0" role="tabpanel" aria-labelledby="kyc-verify-wizard-h-0" class="body current" aria-hidden="false">
                    <div class="row">
                        ...
                        <div class="col-lg-12">
                            <div class="mb-3">
                                <label for="kycfirstname-input" class="form-label">Department Head</label>
                                <input type="text" class="form-control" placeholder="Enter Department Head"
                                    value={formData.deptHead}
                                    onChange={(e) =>
                                        setFormData({ ...formData, deptHead: e.target.value })
                                    } />
                            </div>
                        </div>
                    </div>
